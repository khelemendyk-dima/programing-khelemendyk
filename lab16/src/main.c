...
{
	// memory allocation
	struct List *list = malloc(1 * sizeof(struct List));
	list->size = 0;
	list->head = NULL;
	...
	while (answer != 0) {
		answer = get_answer();
		if (answer == 1) {
			size_t position = get_position_to_add(list->size);
			get_bulb(bulb);
			insert_bulb(list, position, bulb);
		} else if (answer == 2) {
			if (list->size > 0) {
				size_t position = get_position_to_remove(list->size);
				remove_bulb(list, position);
			} else {
				printf("You don't hane any bulb...\n");
			}
		} else if (answer == 3) {
			find_burned_bulbs(list);
		} else if (answer == 4) {
